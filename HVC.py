        ...
        for j in keys[i]:
            ...
            for k in j:
                ...
                if "ЛН" in data_enumeration[k]:
                    ax.plot(data[k][0], data[k][1], label=data_enumeration[k], color=f"C{clor}", linestyle='--')
                else:
                    ax.plot(data[k][0], data[k][1], label=data_enumeration[k], color=f"C{clor}")
            ax.plot([1.8], [77], marker="X", markersize=10, linestyle='None',     label='Раб. Т')
            st.set_legend(ax)
            plt.savefig(f"{export_dir}/{i}{data_enumeration[j[0]]}")
            plt.cla()
        # plt.show()
